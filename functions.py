...
def generate_var_dict(data, mle_dict):
    ...
    if mle_dict['beta-Poisson'][2] < 1e-4:
        ...
        bp_var = sample_mean * (
            1 + mle_dict['beta-Poisson'][1] / (mle_dict['beta-Poisson'][1] + mle_dict['beta-Poisson'][2] + 1))
...
def generate_llh_dict(data, mle_dict):
    ...
    -------
        llh_dict : dictionary
            dictionary containing log likelihood and AIC for each maximum
            likelihood fit
    '''
...
def rp_ci_from_bootstrap_samples(samples, mle, confidence_level):
    '''
    Calculate reverse percentile confidence intervals from a set of bootstrap
    samples.

    Parameters
    ----------
        samples : array
            set of samples of parameter generated by bootstrapping
        mle : float
            maximum likelikelihood estimate of parameter
        confidence_level : float
            confidence level to calculate interval for as a percentage
    ...
    ci=[2 * mle - np.percentile(samples, 100 - alpha/2),
        2 * mle - np.percentile(samples, alpha/2)]

    return ci

def ci_from_bootstrap_samples(samples, mle, confidence_level):
    '''
    Calculate confidence intervals from a set of bootstrap samples.
