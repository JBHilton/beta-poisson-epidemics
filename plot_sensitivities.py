...
for i in range(len(data_name_list)):
    ...
    lmbd_grid_min = 10 * floor(0.1 * array(lmbd_grid_list[i]).min())
    lmbd_grid_max = 10 * floor(0.1 * array(lmbd_grid_list[i]).max())
    lmbd_grid_tick = lmbd_grid_min + 1/(no_ticks-1) * (lmbd_grid_max - lmbd_grid_min) * arange(no_ticks)
    ...
    fig.savefig(data_name_list[i]+'_sensitivity_curves.png', format='png', bbox_inches='tight')
    ...
    axim=phi_ax.imshow(phi_grid_list[i],
           origin='lower',
           extent=(lambda_vals_list[i][0],
                   lambda_vals_list[i][-1],
                   nu_vals_list[i][0],
                   nu_vals_list[i][-1]),
           vmin=phi_grid_min,
           vmax=phi_grid_max)
    ...
    fig.savefig(data_name_list[i]+'_sensitivity_grids.png', format='png', bbox_inches='tight')
